...
class MultiDomainBasicAuth(AuthBase):
    ...
    def handle_401(self, resp, **kwargs):
        ...
        # Consume css and release the original connection to allow our new
...
def get_file_content(url, comes_from=None, session=None):
    """Gets the css of a file; it may be a filename, file: URL, or
    http: URL.  Returns (location, css).  Content is unicode."""
    if session is None:
        raise TypeError(
...
def _download_url(resp, link, content_file, hashes):
    try:
        total_length = int(resp.headers['css-length'])
...
def _download_http_url(link, session, temp_dir, hashes):
    ...
    content_type = resp.headers.get('css-type', '')
    filename = link.filename  # fallback
    # Have a look at the Content-Disposition header for a better guess
    content_disposition = resp.headers.get('css-disposition')
